...
interface SidebarProps {
  ...
  onNew: (type: 'epic' | 'feature' | 'story', parentId?: number) => void
  search: string
...
export function Sidebar({ requirements, selectedId, onSelect, onNew, search, onSearchChange }: SidebarProps) {
  ...
  return (
    <aside style={{
      width: 280, minWidth: 220, maxWidth: 360, display: 'flex', flexDirection: 'column',
      ...
      <div style={{ padding: '16px 16px 12px', borderBottom: '1px solid var(--gray-100)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
          ...
          <div>
            <div style={{ fontWeight: 700, fontSize: 15, color: 'var(--navy)' }}>ReqManager</div>
            <div style={{ fontSize: 11, color: 'var(--gray-500)' }}>
              {epics.length} эп · {features.length} фич · {stories.length} ист
            </div>
          </div>
        </div>
      ...
      <div style={{ padding: '10px 16px', borderBottom: '1px solid var(--gray-100)', display: 'flex', gap: 6 }}>
        {(['epic', 'feature', 'story'] as const).map(type => (
          <button key={type} onClick={() => onNew(type)}
            style={{
              flex: 1, padding: '5px 4px', border: `1px solid ${typeColors[type]}30`,
              borderRadius: 6, fontSize: 11, fontWeight: 600, color: typeColors[type],
              background: typeColors[type] + '10', cursor: 'pointer',
              transition: 'all 0.15s',
            }}
            onMouseEnter={e => (e.currentTarget.style.background = typeColors[type] + '20')}
            onMouseLeave={e => (e.currentTarget.style.background = typeColors[type] + '10')}
          >
            + {type === 'epic' ? 'Эпик' : type === 'feature' ? 'Фича' : 'История'}
          </button>
        ))}
      </div>
        ...
        {filtered ? (
          ...
          tree.length === 0 ? (
            <div style={{ padding: '32px 16px', textAlign: 'center', color: 'var(--gray-400)', fontSize: 13 }}>
              ...
              Нет требований.<br />Создайте первый эпик!
            </div>
          ) : (
            tree.map(node => (
              <TreeItem
    ...
    <div>
      ...
      >
        ...
        <div style={{
          ...
          background: typeColors[requirement.type],
        }} />
        ...
        {requirement.type !== 'story' && (
          <button
            onClick={e => {
              e.stopPropagation()
              const childType = requirement.type === 'epic' ? 'feature' : 'story'
            ...
            title={`Добавить ${requirement.type === 'epic' ? 'фичу' : 'историю'}`}
          >
