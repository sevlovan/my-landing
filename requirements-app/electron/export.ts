...
export async function exportToExcel(requirements: Requirement[], filePath: string): Promise<void> {
  ...
  const sheet = workbook.addWorksheet('Требования СУИД')

  sheet.columns = [
    { header: 'ID', key: 'req_id', width: 14 },
    { header: 'Тип', key: 'type', width: 6 },
    { header: 'Название', key: 'title', width: 50 },
    { header: 'Описание (краткое)', key: 'description', width: 50 },
    { header: 'Описание (полное)', key: 'full_description', width: 60 },
    { header: 'Приоритет', key: 'priority', width: 14 },
    { header: 'Статус', key: 'status', width: 16 },
    { header: 'Ценность', key: 'cenn', width: 30 },
  ...
  sorted.forEach(req => {
    ...
    const bgColor = req.type === 'is' ? 'FFE8EAF6' : req.type === 'bf' ? 'FFF3F4F6' : 'FFFFFFFF'
...
export async function exportToWord(requirements: Requirement[], filePath: string): Promise<void> {
  ...
  const children: any[] = [
    new Paragraph({
      text: 'Реестр функциональных требований СУИД',
  ...
  function addSection(items: Requirement[], heading: string, level: typeof HeadingLevel[keyof typeof HeadingLevel]) {
    ...
    children.push(new Table({
      ...
      rows: [
        new TableRow({
          tableHeader: true,
          children: ['ID', 'Название', 'Статус', 'Автор'].map(h =>
            new TableCell({
        ...
        ...items.map(req =>
          new TableRow({
            children: [
              ...
              new TableCell({
                children: [
                  new Paragraph(req.title),
                  ...(req.description ? [new Paragraph({ children: [new TextRun({ text: req.description, italics: true, size: 18 })] })] : []),
                ],
              }),
  ...
  addSection(isSystems, 'Информационные системы (ИС)', HeadingLevel.HEADING_1)
  addSection(bfBlocks, 'Функциональные блоки (БФ)', HeadingLevel.HEADING_2)
  addSection(ftItems, 'Функциональные требования (ФТ)', HeadingLevel.HEADING_3)
...
export function exportToPdf(requirements: Requirement[], filePath: string): void {
  ...
  doc.setFontSize(16)
  doc.text('Реестр требований СУИД', 14, 16)
  doc.setFontSize(10)
  ...
  ;(doc as any).autoTable({
    startY: 28,
    head: [['ID', 'Тип', 'Название', 'Описание', 'Приоритет', 'Статус', 'Автор']],
    body: sorted.map(r => [
      r.req_id,
      typeLabel(r.type),
      r.title,
...
function statusLabel(s: string) {
  ...
  if (s === 'in_review') return 'На согласовании'
  if (s === 'approved') return 'Согласовано'
  if (s === 'rework') return 'На доработке'
